...
function buildWindowsMasterLauncher(): string {
  const lines = [
    ...
    '  Write-Host "[STAGE] selected-team-lookup"',
    "  $TeamId = ([string]@($TeamIds)[$ArrayIndex]).Trim()",
    "  $HasSavedJql = ([string]$TeamHasJqlById[$TeamId]).Trim()",
    '  Write-Host ("[STAGE] saved-jql-lookup flag={0}" -f $HasSavedJql)',
    '  if ($HasSavedJql -ne "1") {',
    '    Fail-Renew "No real saved JQL found in teams/$TeamId/team.json (savedJqlFlag=$HasSavedJql). Add JQL in the app first, then run this file again."',
    "  }",
    '  $SelectedTeamIds += $TeamId',
    "}",
    "",
    "if (-not $env:JIRA_TOKEN) {",
    '  Write-Host "[STAGE] token-prompt"',
